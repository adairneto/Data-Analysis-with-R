...
samp1 %>%
  summarise(mu_1 = mean(area), pop_med_1 = median(area), 
            sigma_1 = sd(area), pop_iqr_1 = IQR(area),
            pop_min_1 = min(area), pop_max_1 = max(area),
            pop_q1_1 = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3_3 = quantile(area, 0.75))  # third quartile, 75th percentile
```
...
ggplot(data = sample_price50, aes(x = x_bar)) +
  geom_histogram(binwidth = 20)
```
...
# type your code for this Exercise here, and Run Document
sample_price150 <- ames %>%
                    rep_sample_n(size = 150, reps = 5000, replace = TRUE) %>%
                    summarise(x_bar = mean(price))

ggplot(data = sample_price150, aes(x = x_bar)) +
  geom_histogram(binwidth = 20)
